...
def TransferProcess(request, account_number, transaction_id):
    ...
    if request.method == "POST":
        ...
        if 'use_face_verification' in request.POST:
            ...
            return redirect(f"core:face-verification?next={request.path}")
        ...
        if request.session.get('face_verified', False):
            ...
            messages.success(request, "Transfer completed with face verification!")
    ...
    context = {
        "account": account,
        "transaction": transaction,
        "face_verification_enabled": hasattr(request.user, 'kyc') and bool(request.user.kyc.face_encoding)
    }
    return render(request, "transfer/transfer-confirmation.html", context)
