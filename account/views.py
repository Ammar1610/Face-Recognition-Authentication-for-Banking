...
from .face_verification_forms import FaceVerificationForm
...
def face_verification_view(request):
    if not hasattr(request.user, 'kyc') or not request.user.kyc.face_encoding:
        messages.warning(request, "Face verification not available. Please complete KYC first.")
        return redirect('account:kyc-reg')

    if request.method == 'POST':
        form = FaceVerificationForm(request.POST, request.FILES, user=request.user)
        if form.is_valid():
            request.session['face_verified'] = True
            next_url = request.GET.get('next', 'account:dashboard')
            return redirect(next_url)
    else:
        form = FaceVerificationForm(user=request.user)
    
    return render(request, 'account/face_verification.html', {'form': form})
...
def savingprocess(request, account_number, transaction_id):
    ...
    if request.method == "POST":
        # Check for face verification if enabled
        if 'use_face_verification' in request.POST:
            request.session['next_after_face_verify'] = request.path
            return redirect('account:face-verification?next=' + request.path)
        
        if request.session.get('face_verified', False):
            # Face verified, process transaction
            ...
            messages.success(request, "Transaction completed successfully with face verification")
            ...
            if pin_number == account.pin_number:
                ...
                messages.success(request, "Transaction completed successfully with PIN")
                return redirect("account:save-completed", account.account_number, transaction.transaction_id)
            else:
...
def withdrawprocess(request, account_number, transaction_id):
    ...
    if request.method == "POST":
        ...
        if request.session.get('face_verified', False):
            ...
            messages.success(request, "Withdrawal completed successfully with face verification")
            ...
            if pin_number == account.pin_number:
                ...
                messages.success(request, "Withdrawal completed successfully with PIN")
                return redirect("account:withdraw-completed", account.account_number, transaction.transaction_id)
            else:
                messages.warning(request, "Incorrect PIN")
