...
# edited from here for face recognition
...
class DollarAccount(models.Model):
    ...
    date= models.DateTimeField(auto_now_add=True)
    dollar_rate = models.DecimalField(max_digits=12, decimal_places=2, default=87.07)
...
class KYC(models.Model):
    ...
    face_encoding = models.TextField(blank=True, null=True) #added for face recognition 
    ...
    def update_face_encoding(self):
        try:
            ...
            if os.path.exists(img_path):
                ...
                if len(face_locations) == 1:  # We want exactly one face
                    encoding = face_recognition.face_encodings(image, known_face_locations=face_locations)[0]
                    self.face_encoding = ','.join(map(str, encoding))
                    # Use update to avoid recursive save
                    KYC.objects.filter(pk=self.pk).update(face_encoding=self.face_encoding)
                else:
                    # Handle cases with 0 or multiple faces
                    pass
